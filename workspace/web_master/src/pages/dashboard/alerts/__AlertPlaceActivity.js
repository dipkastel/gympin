...
const __AlertPlaceActivity = (props) => {
    ...
    return (
        <>
            {place.Status == "ACTIVE" ? <Card variant={"outlined"} sx={{borderRadius: 3, margin: 1}}>
                <CardHeader
                    component={"a"}
                ...
                sx={{m: 2}}
                severity={"error"}
                action={<Button color={"error"} variant={"contained"} onClick={e => changePlaceStatus("ACTIVE")}>
                    فعالسازی
                </Button>}
            >
...
export default connect(null, sagaActions)(__AlertPlaceActivity)
