    ...
    try {
      ...
      login({
        ...
        .catch((err) => {
          ...
          setStatus(statusEnum.smsSent);
          setCodeError(
            err?.response?.data?.Message || "کد وارد شده معتبر نیست.",
          );
        });
    } catch (e) {}
  }
            ...
            <CardContent sx={{ px: 5, pb: "80px !important" }}>
              <form onSubmit={submitForm}>
                <Grid container direction={"column"}>
                  ...
                  <Typography sx={{ mt: 7 }} variant={"subtitle1"}>
                    ورود با کد یکبار مصرف
                  </Typography>
                  <Typography
                    sx={{ mt: 7, textAlign: "start" }}
                    variant={"body2"}
                  >
                  ...
                  {(status === statusEnum.smsSent ||
                    status === statusEnum.loadingConfirm) && (
                    <>
                      <Typography
                        sx={{ mt: 4, textAlign: "start" }}
                        variant={"body2"}
                      >
                        لطفا کد دریافت شده در پیامک را وارد نمایید
                      </Typography>
                      <ValidatedTextField
                        label="کد دریافتی"
                        variant="outlined"
