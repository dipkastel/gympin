...
const ChatWidget = () => {
  ...
  const changeMessages = useCallback((msg) => {
    ...
    setPendingMessages((prev) =>
      prev.filter(
        (pm) => pm.Message !== msg.Message || pm.Sender !== msg.Sender,
      ),
    );
  }, []);
  ...
  const deletePendingMessage = useCallback(
    (msg) => () => {
      setPendingMessages((prev) => prev.filter((pm) => pm.id !== msg.id));
    },
    [],
  );
  ...
  const resendPendingMessage = useCallback(
    (msg) => () => {
      setPendingMessages((prev) =>
        prev.map((pm) => (pm.id === msg.id ? { ...pm, failed: false } : pm)),
      );
          ...
          >
            {[...messages, ...pendingMessages].map((msg, i) => (
              <Box
                ...
                sx={{
                  alignSelf:
                    msg?.Sender === "Client" ? "flex-start" : "flex-end",
                  bgcolor:
                    msg?.Sender === "Client"
                      ? pendingMessages.includes(msg)
                        ? "#ff9999"
                        : "primary.light"
                      : "grey.300",
                  color: "black",
              ...
              >
                ...
                {msg?.failed && (
                  <>
                    <Delete
                      sx={{ fontSize: 16, color: "#4b0505" }}
                      onClick={deletePendingMessage(msg)}
                    />
                  </>
                )}
              </Box>
