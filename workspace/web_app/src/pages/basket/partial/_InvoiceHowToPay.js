...
const _InvoiceHowToPay = ({userBasket,setUserCanPay,invoiceCredits,SetInvoiceCredits}) => {
    ...
    return (
        <>
            <Card elevation={3} sx={{m: 1}}>
                <CardHeader sx={{backgroundColor:"#969696",p:"8px !important",color:"#ffffff"}} title={"نحوه پرداخت"} />
                {userCredits&&userBasket&&<CardContent>
                    {invoiceCredits && getPayableCreditsOfUser().map((item, number) => (
                        <Grid
                            ...
                            {(getPayableCreditsOfUser().length-1>number)&&<IconButton aria-label="down" color={"inherit"}
