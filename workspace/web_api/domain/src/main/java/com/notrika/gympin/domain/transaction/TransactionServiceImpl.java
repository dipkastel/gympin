...
public class TransactionServiceImpl extends AbstractBaseService<TransactionParam, TransactionDto, TransactionQuery, TransactionEntity> implements TransactionService {
    ...
    public String setPaymentRequest(PaymentRequestParam param) {
        ...
        if (param.getSelectedPaymentType() == 80L) {
            ...
            requestData.setOrderId(transaction.getId());
            requestData.setAmount(param.getAmount().longValue());
            requestData.setCallBackUrl("https://web.gympin.ir/checkout/");
            ...
            SalePaymentRequest salePaymentRequest = new SalePaymentRequest();
            salePaymentRequest.setRequestData(requestData);
            var gatwayresult =  gatewayParsianService.salePaymentRequest(salePaymentRequest).getSalePaymentRequestResult();
            if(gatwayresult.getStatus()==0&&gatwayresult.getToken()>0){
                result = "https://pec.shaparak.ir/NewIPG/?token="+gatwayresult.getToken();
            }else{
                throw new GatewayIsNotAvalable();
            }
        } else if (param.getSelectedPaymentType() == 81L) {
            var transaction = submitTransAction("پرداخت از درگاه -- ",param,serial);
            ...
            var transaction = submitTransAction("پرداخت کارت به کارت با شماره تراکنش : " + param.getTransactionReference(), param, serial);
            result = serial.split("-")[0];
        } else if (param.getSelectedPaymentType() == 95L) {
            var transaction = submitTransAction("پرداخت بانکی با شماره تراکنش : " + param.getTransactionReference(), param, serial);
            result = serial.split("-")[0];
        } else if (param.getSelectedPaymentType() == 98L) {
            var transaction = submitTransAction("پرداخت چک با شماره سریال :" + param.getTransactionReference() + " و تاریخ :" + param.getChequeDate(), param, serial);
            result = serial.split("-")[0];
        } else if (param.getSelectedPaymentType() == 99L) {
            UserEntity user = (UserEntity) GympinContextHolder.getContext().getEntry().get(GympinContext.USER_KEY);
            var transaction = submitTransAction("درخواست افزایش اعتبار از پنل توسط : "+user.getId()+" - "+ user.getUsername(), param, serial);
            result = serial.split("-")[0];
        } else {
            throw new unknownPaymentType();
        }
