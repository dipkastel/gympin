...
import static com.notrika.gympin.common.purchased.purchasedSubscribe.enums.PurchasedStatus.COMPLETE;
import static com.notrika.gympin.common.purchased.purchasedSubscribe.enums.PurchasedStatus.EXPIRE;
...
public class PurchasedSubscribeServiceHelper {
    ...
    public boolean checkForAccess(PurchasedSubscribeEntity purchesedSubscribe, Long placeId) {
        ...
        var halls = purchesedSubscribe.getTicketSubscribe().getActiveTimes().stream().map(TicketSubscribeHallActiveTime::getHall).collect(Collectors.toSet());
        for (var hall : halls) {
