...
public interface TicketFoodMenuRepository extends BaseRepository<TicketFoodMenuEntity, Long> {
    ...
    @Query(value = "SELECT tfm.date FROM TicketFoodMenuEntity tfm JOIN TicketFoodItemEntity  tfi on tfm.foodItem.id = tfi.id WHERE tfi.place.id = :#{#cateringId} and tfm.deleted = false and tfi.deleted = false GROUP BY tfm.date order by tfm.date desc")
