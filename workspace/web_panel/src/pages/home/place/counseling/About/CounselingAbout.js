...
const CounselingAbout = ({counseling}) => {
    const error = useContext(ErrorContext);
    const [counselingAbouts, SetCounselingAbouts] = useState([])
    const [openModalAdd, setOpenModalAdd] = useState(false)
    const [itemToDelete, setItemToDelete] = useState(null)
    const [itemToEdit, setItemToEdit] = useState(null)
    useEffect(() => {
        getAboutsOfCounseling();
    }, []);

    function getAboutsOfCounseling() {
        placeAbout_getByPlace({Id: counseling.Id}).then(data => {
            SetCounselingAbouts(data.data.Data);
        }).catch(e => {
                    try {
