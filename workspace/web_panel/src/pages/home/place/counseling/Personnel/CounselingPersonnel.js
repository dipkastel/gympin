...
const CounselingPersonnel = ({counseling}) => {
    const error = useContext(ErrorContext);
    const history = useHistory();
    const [counselingPersonnels,SetCounselingPersonnels] = useState([])
    const [openModalAdd,setOpenModalAdd] = useState(false)
    const [itemToDelete,setItemToDelete] = useState(null)
    useEffect(() => {
        getPersonnelsOfCounseling();
    }, []);
    function getPersonnelsOfCounseling(){
        placePersonnel_ByPlace({Id:counseling.Id}).then(data=>{
            SetCounselingPersonnels(data.data.Data);
        }).catch(e => {
            try {
    ...
    function renderModalAdd() {
        ...
        function addPerson(e) {
            e.preventDefault()
            placePersonnel_add({Place:{Id:counseling.Id},PhoneNumber:e.target.PhoneNumber.value})
                .then(data=>{
...
export default CounselingPersonnel;
