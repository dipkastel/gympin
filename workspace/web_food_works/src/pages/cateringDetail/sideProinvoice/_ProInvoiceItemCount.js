        ...
        <ButtonGroup sx={{ mt: 1 }} variant={"outlined"} size={"small"}>
          <Button
            sx={{
              ...
              borderRightColor: "#66bb6a80 !important",
            }}
            color={"success"}
            onClick={(e) =>
              item.Count == itemInValue
                ? inAddOrder(item)
                : inSetOrderCount(item)
            }
          >
          ...
          <TextField
            ...
            aria-valuemax={999}
            type={"number"}
            inputProps={{ className: "px-0 py-2 text-center" }}
            className={"count"}
          />
          <Button color={"error"} onClick={(e) => inRemoveOrder(item)}>
            {item.Count < 2 ? <DeleteOutline /> : <Remove />}
          </Button>
        </ButtonGroup>
      ) : (
        <>
