...
const _CateringSelectDate = ({ selectedDate, setSelectedDate, catering }) => {
  ...
  useEffect(() => {
    ...
    for (
      var i = catering.LastOrderDayCount;
      i < 30 + catering.LastOrderDayCount;
      i++
    ) {
  ...
  function getActiveDates() {
    TicketFoodMenu_getDates({ cateringId: catering.Id })
      .then((result) => {
  ...
  function getLabel(itemDate) {
    return (
      <Grid
        sx={{ p: 2, borderRadius: 3, border: "1px solid" }}
        disabled={isDisable(itemDate)}
      >
        <Typography
          sx={{ textDecoration: isDisable(itemDate) ? "line-through" : "none" }}
          variant={"h6"}
        >
          {" "}
